# SLE

> 星闪低延迟（SparkLink Low Energy）领域完整应用开发参考。

---

## 快速导航

### 概述

- [概述](./overview/index.md)

### 基础入门

- [Hello SLE (SparkLink Low Energy)](./basics/hello-connect.md)
- [通知推送（Notify）](./basics/hello-notify.md)
- [属性读写（Read/Write）](./basics/hello-readwrite.md)

### 数据通信

- [UART (Universal Asynchronous Receiver/Transmitter) 透传](./data-comm/uart-bridge.md)
- [传感器上报](./data-comm/sensor-report.md)
- [设备配置](./data-comm/device-config.md)
- [高吞吐传输](./data-comm/high-throughput.md)
- [分片传输](./data-comm/fragmentation.md)

### 连接与链路管理

- [连接参数动态更新](./link-mgmt/conn-param-tuning.md)
- [无线链路自适应（PHY/MCS）](./link-mgmt/phy-mcs-switch.md)
- [RSSI (Received Signal Strength Indicator) 测距](./link-mgmt/rssi-ranging.md)
- [多连接](./link-mgmt/multi-connection.md)

<!--
### 安全

- [SLE 安全机制](./security/just-works-pairing.md)

### 垂直应用场景

- [Wi-Fi 配网](./verticals/wifi-config.md)
- [OTA (Over-The-Air)](./verticals/ota.md)
- [CHBA (Converged Host Bus Adapter) IP (Internet Protocol)](./verticals/chba-ip.md)
- [HID (Human Interface Device) 鼠标](./verticals/hid-mouse.md)
- [HID 键盘](./verticals/hid-keyboard.md)
- [HADM (High Accuracy Distance Measurement) 测距](./verticals/hadm-ranging.md)
- [网关](./verticals/gateway.md)

### 测试与调试

- [吞吐量测试](./test-debug/throughput-test.md)
- [RF (Radio Frequency) 认证测试](./test-debug/rf-cert-test.md)
- [故障排除](./test-debug/troubleshooting.md)
-->

