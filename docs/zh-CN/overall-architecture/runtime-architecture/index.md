...
> 基于 `[CHIP_NAME]-liteos-app` 当前源码，说明 LiteOS (Huawei LiteOS) 开始调度后系统中的任务组成、执行上下文、通信边界和资源约束
...
平台任务表定义在 `application/[CHIP_NAME]/[CHIP_NAME]_liteos_application/main.c` 的 `g_app_tasks[]` 中。实际固件只包含当前 target 和 Kconfig 选中的任务，并非下表任务会同时存在。

| 运行域 | 典型任务 | 启用条件 | 运行时职责 |
|--------|----------|----------|------------|
| 系统监控 | `app` | 非 `CHIP_EDA` 构建 | 完成时钟校准，并周期输出内存、日志丢弃和 AT 接收统计 |
| 诊断与控制 | `cmd_loop`、`log`、`at` | `TEST_SUITE`、`HSO_SUPPORT`、`AT_COMMAND` | 处理测试命令、诊断日志和 AT 消息 |
| Wi-Fi | `wifi` | `WIFI_TASK_EXIST` | 运行 Wi-Fi Host 协议处理逻辑 |
| BLE (Bluetooth Low Energy) / SLE (SparkLink Low Energy) | `bt` | `BGLE_TASK_EXIST` | 运行 BLE / SLE 协议处理逻辑 |
| BR (Basic Rate) / EDR (Enhanced Data Rate) | `bt_sdk`、`bth_sdk`、`bt_service` | `BTH_TASK_EXIST` | 处理蓝牙 SDK 消息、数据和服务流程 |
| Radar | `radar_driver`、`radar_feature` | `CONFIG_RADAR_SERVICE` 且未定义 `DISABLE_RADAR` | 分别处理雷达驱动和特征计算 |
| 生态组件 | `hilink`、`matter` | `CONFIG_SUPPORT_HILINK`、`CONFIG_SUPPORT_MATTER` | 运行对应生态组件的主处理逻辑 |
...
当前 LiteOS 任务优先级范围为 0～31，数值越小优先级越高。`main.c` 中的 `TASK_PRIORITY_*` 和各任务栈宏给出了平台任务的当前配置，例如 `bt` 和 `cmd_loop` 使用较高优先级，`wifi`、`log` 和系统监控任务使用相对较低优先级。

开发者新增或调整任务时应注意：

1. 不要仅为“运行更快”而设置更小的优先级数值。持续运行的高优先级业务任务可能使 Wi-Fi、蓝牙、日志或系统监控任务得不到调度。
2. 使用 OSAL 等待、队列或事件让任务在无工作时阻塞，不使用无延时的永久轮询。
3. 根据最大调用深度、局部变量和所调用协议库确定栈大小；任务栈会占用运行时 RAM (Random Access Memory) 。
4. 检查任务创建和优先级设置返回值。平台任务创建失败时会输出 `create[...] failed`，优先级设置失败时会输出 `set_pri[...] failed`。
...
普通芯片构建中的 `app` 任务会周期输出 `[SYS INFO]`，包含系统内存使用量、日志丢弃数量和 AT 接收计数，可用于观察长期运行趋势。需要进一步定位时，可结合 LiteOS 任务/CPU 统计、内存信息、串口日志、异常信息以及 `.map`、`.elf` 分析：
